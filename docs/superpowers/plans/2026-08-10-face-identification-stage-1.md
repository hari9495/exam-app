...
- Produces: `assessFaceQuality(result): QualityVerdict` where
...
MediaPipe landmarks are already normalised to 0..1 of the frame, so no frame dimensions are
needed — every ratio here is scale-free.
...
Then add the capture hook below the component in the same file. It owns the camera and the
MediaPipe loop; keeping it out of the component body is what lets the four tests above drive the
whole state machine without a webcam.
...
`MEDIAPIPE_WASM_URL` and `FACE_LANDMARKER_MODEL_URL` already exist in
`apps/web/lib/hooks/useWebcamMonitor.ts`. Read that file, and if they are module-local, export them
there and import them here rather than re-declaring the paths — two copies of a model URL is how one
of them silently becomes a CDN later.

The four tests in Step 2 only render the `consent` and `blocked` phases, so `useEnrolmentCapture`
never runs under jsdom and needs no mocking.
