...
**SCOPE ADDENDUM (approved after initial design presentation):** a separate, pre-existing bug in the same file — `MonitoringGateway.tickPresence()` (`apps/exam-runtime/src/monitoring/monitoring.gateway.ts:124`) reads `this.server.sockets.adapter.rooms`, but `this.server` is a Socket.IO `Namespace` instance (this gateway uses `@WebSocketGateway({ namespace: '/monitoring' })`), and on a `Namespace`, `.sockets` is a `Map<SocketId, Socket>` with no `.adapter` property — the adapter lives directly on the namespace as `this.server.adapter`. This throws `TypeError: Cannot read properties of undefined (reading 'rooms')` every 15 seconds (`PRESENCE_TICK_MS`), caught and logged but never fixed, meaning the `roster:presence` broadcast to staff dashboards has likely never worked at runtime. Unrelated to the Express-5 regression (this one throws-and-catches rather than hangs), but fixed as part of this same work since it's in the identical file/feature area and this plan already requires full e2e verification of `MonitoringGateway`. Fix: `this.server.sockets.adapter.rooms` → `this.server.adapter.rooms`.
...
## 6. POST-IMPLEMENTATION CORRECTION (2026-07-12): the diagnosis this spec was built on was wrong

Sections 1-2's "Express 5 router race" mechanism — inherited from Phase 6e's diagnosis and refined during this spec's own scoping — was disproven during implementation. The middleware fix it prescribed was implemented twice (once in `main.ts`, once via `AppModule.configure()`), had zero effect on the e2e failure both times, and was ultimately reverted as unnecessary.

**Actual root cause (empirically confirmed):** Phase 6e's multi-round install churn changed npm's hoisting layout — `@nestjs/websockets`, `@nestjs/platform-socket.io`, and `socket.io` moved from root-hoisted (confirmed present at root in the `pre-dependency-remediation` tag's lockfile) to nested under `apps/exam-runtime/node_modules` only. `apps/api` declares none of them, so when its e2e harness (`test/dual-app.ts`) boots the runtime app in-process, `apps/api`'s own nested `@nestjs/core` cannot resolve `@nestjs/websockets` — and NestJS **silently skips SocketModule registration** when that optional load fails. No Socket.IO server ever attached; every `/socket.io/` request (any transport) fell through to Express's router and 404'd. That 404 is what Phase 6e's diagnosis — whose diagnostic script also ran from `apps/api` and hit the same resolution gap — misread as Express 5 "winning a race." `engine.io`'s `attach()` in fact captures and replaces all prior `'request'` listeners and registers its own `'upgrade'` listener, making the theorized race structurally impossible.

**Consequently, production was never broken**: the real `main.ts` bootstrap resolves modules from `apps/exam-runtime`'s own location, where the stack was always reachable. Bug #6036's "production-impacting" severity was based on the mis-diagnosis; the genuine impact was e2e-harness-only.

**Actual fix (commit 7b9c568):** declare `@nestjs/websockets@^11.1.28` and `@nestjs/platform-socket.io@^11.1.28` as devDependencies of `apps/api` — an honest declaration of what its e2e harness genuinely imports transitively — which also restored single root-hoisted copies. All 4 live-monitoring e2e tests pass in ~10s with zero source-code changes. Sections 4-5's verification approach and the Task 2 (`tickPresence()`) and Task 3 (`@types/express`) scope items remain valid as written.
