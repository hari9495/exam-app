...
- Exact numeric limits (5/60s, 30/60s, 10/60s, 100/60s) are a reasonable starting point, not empirically tuned against real traffic — revisit if they prove too strict/loose once observed in practice. The final review specifically flagged `attempt/proctoring-event`'s 30/60s tier as worth watching: a candidate rapidly switching tabs (the abuse proctoring exists to detect) could exceed it and have integrity events dropped.
...
- Whether the test environment needs throttling disabled/relaxed is genuinely unknown until the existing suites are run against the new global guard — flagged as a real risk to check early in implementation, not deferred silently. (Resolved during implementation: relaxed under `NODE_ENV === 'test'` in each app's `rate-limit-tiers.ts`; the real limits are proven by a live manual check against a production-style boot instead.)
- **Trust proxy / IP keying (deferred, decided at final review):** the throttler keys on `req.ip`, and neither app configures Express `trust proxy`. If either app is ever deployed behind a reverse proxy or load balancer, every client will share the proxy's IP and the strict tiers will collapse to platform-wide limits (e.g. 5 logins/minute for everyone). This repo currently pins no production deployment topology, so the decision is deferred as a **deployment precondition**: when topology is established, either set `trust proxy` to a known hop count (never `true`, which lets clients spoof `X-Forwarded-For` to evade limits) or document that the listeners are directly exposed. Tracked as an ADO follow-up under Epic #6013.

## 6. Post-review addendum (2026-07-13): Redis failure mode is fail-open

The final whole-phase review identified that `ThrottlerGuard` awaits the Redis-backed storage on every request with no fallback — a Redis outage would have returned 500 on every route in both apps (including in-progress exams in `apps/exam-runtime`, which had no Redis dependency at all before this phase). Decision: **fail open**. Each app registers a `FailOpenThrottlerGuard` (subclass of `ThrottlerGuard`) as its `APP_GUARD`: storage failures allow the request through with a logged warning, while `HttpException` rejections (including the genuine 429 `ThrottlerException`) still propagate. Unit-tested for all three paths (fail-open on storage error, 429 rethrow, pass-through). Rationale: the rate limiter is a protection layer, not a dependency the whole platform should die for — availability wins during a Redis outage, at the accepted cost of unthrottled traffic for the outage's duration.
