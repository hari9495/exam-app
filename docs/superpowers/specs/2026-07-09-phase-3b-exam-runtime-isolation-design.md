...
**A 4th case, discovered during implementation (Task 4):** `ExamsService.getResults` (staff-side, staying in `apps/api`) also calls `AttemptSettlementService.settleIfExpired` directly — to lazily finalize an expired `in_progress` attempt so a recruiter viewing results sees an up-to-date status instead of a stale one. This wasn't caught during design because `exams/` was never one of the folders inventoried against the moving modules. Resolved the same way as the other 3: a 4th internal endpoint, `POST /internal/attempts/:id/settle-if-expired`, added to `InternalController`. Unlike `forceSubmit`/`reanalyze`, this one needs no org-ownership re-check in `apps/api` before calling it — `getResults` already validated the exam belongs to the caller's org and derived the attempt id from that exam's own invitations before ever calling the internal client. `ExamRuntimeInternalClient` moved out of `attempts-admin/` into its own small `apps/api/src/exam-runtime-client/` module (with an `ExamRuntimeClientModule` wrapper) once a second consumer (`ExamsModule`) needed it too.
...
**Phase 3c entry criteria, flagged by the final whole-branch review** — none of these block this sub-phase (everything is localhost-only, no real deployment exists yet per Section 1), but they are load-bearing assumptions that real deployment/scale work must not silently violate:

- **The internal HTTP surface has no network isolation of its own** — `InternalModule` is mounted in the same `NestFactory` app, same port, same `api/v1` prefix as the public candidate routes; only the shared-secret header separates `/internal/*` from the public surface. Since the internal controller reads with a full super-admin RLS bypass and no org scoping (by design — the caller already authorized), a leaked secret means force-submit/settle/reanalyze against *any* attempt in *any* org. Phase 3c must bind this surface to a separate listener/port or block `/api/v1/internal/*` at the network edge before any real cloud deployment — never rely on the shared secret as the only boundary once this is network-reachable.
- **`ExamRuntimeInternalClient`'s `fetch` calls have no timeout/abort handling.** If `apps/exam-runtime` hangs, the calling admin request hangs with it; if it's down, the connection error bypasses `throwIfNotOk` (which only inspects a returned `Response`) and surfaces as an opaque 500. Add an `AbortController` timeout and explicit connection-error translation before this crosses a real network boundary in 3c.
- **`ExamsService.getResults` settles expired attempts via sequential, one-at-a-time internal HTTP calls** — fine at today's scale (one process, one exam, a handful of in-progress attempts), but N serial round-trips per results-fetch will not hold up under 3c's load-testing goals. Revisit as a batch endpoint or bounded-concurrency fan-out.
- `InternalAuthGuard`'s secret comparison (`!==`) is not constant-time — irrelevant on localhost, worth revisiting only if this surface becomes network-adjacent.
