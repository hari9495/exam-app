...
- **Cross-service HTTP calls between `apps/api` and `apps/exam-runtime`.** Not needed — both read/write the same shared database directly. Introducing a synchronous cross-service call anywhere on the candidate's hot path would defeat the point of this split.
