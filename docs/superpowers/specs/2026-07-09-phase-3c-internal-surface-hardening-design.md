...
**Phase 3d entry criteria, flagged by this phase's own final whole-branch review** — none block this sub-phase, but they are load-bearing assumptions a future phase must not silently violate:

- **The new `/monitoring-relay/*` endpoint (added mid-phase to fix the monitoring-bridge bug) partially undercuts this phase's own network-isolation goal.** It's registered on the *public* app, which binds to `0.0.0.0`, and is guarded only by the same shared secret as the rest of the internal surface. A leaked secret now grants an off-host caller the ability to inject arbitrary `attempt:status`/`message:sent` events into any exam's recruiter monitoring room — bounded impact (spoofing the live-monitoring UI, no data exfiltration), but a real integrity surface this phase's `127.0.0.1` binding was specifically meant to close for the rest of the internal surface. Phase 3d must either move this relay to its own loopback-bound listener or upgrade its auth beyond the shared secret.
- **The internal app's `127.0.0.1` default bind host has no automated regression guard.** The only proof it isn't `0.0.0.0` today is the manual `curl`-based check in Task 6 (Section 6 above), a one-time confirmation, not a standing test. A future edit to the `?? '127.0.0.1'` default (in `main.ts`) could silently expose the internal surface with no test failure. Phase 3d should add an assertion (e.g. on `internalApp.getHttpServer().address().address`) that fails loudly if this default ever changes.
