...
- The prompt states the topic, difficulty, and requested type mix, and explicitly tells the model this project's own type rules (`single_mcq` → exactly 1 correct option; `multi_mcq` → at least 1 correct option; `true_false` → exactly 2 options, exactly 1 correct) so most output should already pass validation cleanly.
