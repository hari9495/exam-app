...
4. **Group definitions (exact):**
   ...
   - **Interviews today** — the user's panelist interviews, `status = 'confirmed'`, confirmed slot `startsAt` within [dayStart, dayEnd) in the user's timezone. Soonest first.
...
5. **Gating:** identical to `GET /dashboard/summary` — same controller, same class-level `JwtAuthGuard` + `PermissionsGuard`, no extra permission. A user with no interviews/approvals simply gets empty groups.
