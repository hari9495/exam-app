...
export function QuestionListRow({ question }: { question: Question }) {
  ...
  return (
    <div className="group border-b border-recruiter-border last:border-b-0">
      <div className="flex items-center gap-3 px-3 py-2.5">
        ...
        >
          {expanded ? (
            <ChevronDown size={14} className="shrink-0 text-recruiter-text-tertiary" aria-hidden="true" />
          ) : (
            <ChevronRight size={14} className="shrink-0 text-recruiter-text-tertiary" aria-hidden="true" />
          )}
          ...
          <span className={clsx('text-sm text-recruiter-text', !expanded && 'truncate')}>{question.text}</span>
        </button>
        <StatusBadge tone={TYPE_TONE[question.type] ?? 'neutral'}>{TYPE_LABEL[question.type] ?? question.type}</StatusBadge>
        <span className="hidden w-16 shrink-0 text-xs text-recruiter-text-secondary sm:block">
          {DIFFICULTY_LABEL[question.difficulty] ?? question.difficulty}
        </span>
        <span className="w-12 shrink-0 text-right text-xs text-recruiter-text-tertiary">{question.marks}</span>
        <Link
        ...
        <div className="flex flex-col gap-2 px-3 pb-3 pl-9">
          ...
          {tags.length > 0 && (
            <div className="flex flex-wrap items-center gap-1">
              {tags.map((tag) => (
                <span key={tag.id} className="rounded bg-recruiter-bg-subtle px-1.5 py-0.5 text-xs text-recruiter-text-tertiary">
                  {tag.name}
                </span>
              ))}
            </div>
