...
export function Dialog({
  ...
  if (!open) return null;
  return (
    <div onClick={onClose} className="v2-overlay-in" style={{ position: 'fixed', inset: 0, background: 'rgba(11,18,32,.45)', display: 'grid', placeItems: 'center', zIndex: 100, padding: 20 }}>
      ...
      <div role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()} className="v2-panel-in" style={{ display: 'flex', flexDirection: 'column', width: '100%', maxWidth: width, maxHeight: 'calc(100vh - 40px)', overflow: 'hidden', background: 'color-mix(in srgb, var(--ink) 6%, var(--paper))', border: '1px solid var(--hair)', borderRadius: 16, boxShadow: '0 24px 60px -20px rgba(11,18,32,.5)' }}>
