...
// v2 WhatsApp Templates — mirrors message-templates/page.tsx (shared DataTable + a v2 edit
// dialog) minus subject (WhatsApp is body-only). Same hooks/dialog structure, swapped to the
// WhatsApp template hooks + the WhatsApp-configured banner.
import { useState } from 'react';
import type { ColumnDef } from '@tanstack/react-table';
import { MoreHorizontal, Pencil, RotateCcw, Plus, Trash2 } from 'lucide-react';
import { useAuth } from '../../../../lib/auth-context';
import { useWhatsappTemplates, useUpsertWhatsappTemplate, useSetWhatsappTemplateEnabled, useDeleteWhatsappTemplate } from '../../../../lib/hooks/useCandidateWhatsapp';
import { usePipelines } from '../../../../lib/hooks/usePipelines';
import { useWhatsappConfig } from '../../../../lib/hooks/useWhatsappConfig';
import { type WhatsappTemplate, type Pipeline } from '../../../../lib/types';
import { DataTable, DT_FEATURES, dt, SortHead, Cb } from '../../../../components/ui-v2/DataTable';
...
// Stage NAMES that ship a built-in default (see
// apps/api/src/candidate-whatsapp/default-whatsapp-templates.ts, keyed by name since defaults
// aren't stored per-org). A saved template whose triggerStageId resolves to one of these names
// can be "restored" to its built-in copy; anything else has nothing to fall back to, so it's a
// plain delete.
...
const BLANK_TEMPLATE: WhatsappTemplate = { id: null, name: '', triggerStageId: null, triggerMode: 'manual', body: '', enabled: true, isDefault: false };
...
function EditTemplateDialog({ template, pipelines, isNew = false, onClose, onSaved }: { template: WhatsappTemplate; pipelines: Pipeline[]; isNew?: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  function handleSave(e: React.FormEvent) {
    e.preventDefault();
    if (!canSave) { setError('Name and message are required.'); return; }
    setError(null);
    upsert.mutate(
    ...
    <Dialog open onClose={onClose} title={isNew ? 'New template' : `Edit "${template.name}"`} width={560}>
      <form onSubmit={handleSave}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <TextField id="wtpl-name" label="Name" value={name} onChange={setName} required autoComplete="off" />
          ...
          <div>
            <label htmlFor="wtpl-body" className="v2-label">Message</label>
            ...
            <textarea id="wtpl-body" value={body} onChange={(e) => setBody(e.target.value)} rows={6} maxLength={4096} style={{ width: '100%', boxSizing: 'border-box', padding: '9px 11px', fontSize: 13, borderRadius: 8, border: '1px solid color-mix(in srgb, var(--ink) 15%, var(--hair))', background: 'var(--paper)', color: 'var(--ink)', outline: 'none', resize: 'vertical', fontFamily: 'inherit', lineHeight: 1.5 }} />
          </div>
        </div>
...
export default function V2WhatsappTemplatesPage() {
  ...
  const setEnabled = useSetWhatsappTemplateEnabled();
  ...
  const showNotConfiguredBanner = whatsappConfigLoaded && (whatsappConfig?.configured === false || whatsappConfig?.whatsappEnabled === false);

  const q = search.trim().toLowerCase();
  const rows = q ? (templates ?? []).filter((t) => t.name.toLowerCase().includes(q)) : (templates ?? []);

  function handleToggleEnabled(t: WhatsappTemplate, next: boolean) {
    if (!t.id) return;
  ...
  function handleRestoreDefault(t: WhatsappTemplate) {
    if (!t.id) return;
  ...
  function handleDelete(t: WhatsappTemplate) {
    if (!t.id) return;
  ...
  const columns: ColumnDef<typeof DT_FEATURES, WhatsappTemplate>[] = [
    ...
    <>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 16 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--muted)', margin: '4px 0 0', maxWidth: 620 }}>Control what candidates are sent over WhatsApp at each pipeline stage. Edit a default template to override it for your organization, or restore it to fall back to the built-in copy. Add a manual-only template for ad-hoc messages you send yourself.</p>
        </div>
        <Button onClick={() => setCreating(true)}><Plus size={15} /> New template</Button>
      </div>

      {showNotConfiguredBanner && (
        <div style={{ marginBottom: 12, fontSize: 13, padding: '9px 13px', borderRadius: 9, border: '1px solid color-mix(in srgb, #a16207 30%, transparent)', background: 'color-mix(in srgb, #a16207 8%, transparent)', color: '#a16207' }}>WhatsApp messages won&apos;t send until a provider is configured and enabled in Organization settings.</div>
      )}
      {notice && (
