...
// v2 panel exam report — same as the recruiter v2 exam report (summary tiles + Candidates/Accuracy
// tabs, all hooks/filters/export/compare verbatim, format only), but every internal link/route
// stays under /v2/panel/* so panelists don't cross into the recruiter console. Reuses
// QuestionAccuracyPanel + IntegrityBadge as-is.
...
export default function V2PanelExamReportPage() {
  ...
  const columns: ColumnDef<typeof DT_FEATURES, ExamResultRow>[] = [
    ...
    { id: 'name', accessorFn: (r) => r.candidateName.toLowerCase(), header: sortHead('Candidate'), cell: ({ row }) => <Link href={`/v2/panel/reports/${examId}/candidates/${row.original.candidateId}?attemptId=${row.original.attemptId ?? ''}`} style={{ fontWeight: 500, color: 'var(--org-primary)', textDecoration: 'none' }}>{row.original.candidateName}</Link> },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: 10 }}>
            <div style={{ minWidth: 220 }}>
              ...
              <ExamPicker width="100%" value={examId} initialLabel={exam?.title} onChange={(nextExamId) => nextExamId && nextExamId !== examId && router.push(`/v2/panel/reports/${nextExamId}`)} />
            </div>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button type="button" className="v2-hoverbtn" style={{ ...dt.primaryBtn, opacity: selectedIds.length < 2 ? 0.5 : 1, cursor: selectedIds.length < 2 ? 'not-allowed' : 'pointer' }} disabled={selectedIds.length < 2}
                onClick={() => router.push(`/v2/panel/reports/${examId}/compare?invitationIds=${selectedIds.join(',')}`)}>Compare selected</button>
            </div>
          </div>
