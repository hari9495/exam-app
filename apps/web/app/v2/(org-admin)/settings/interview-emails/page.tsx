...
// v2 Settings -> Interview Emails. Org-admin editor for the eight interview notification emails
// (candidate invite, panelist assignment, the three confirmation notices, decline/reschedule
// recruiter notices, and cancellation). Each event is an independently-saved slot: subject + body +
// enabled, PUT one at a time via useUpsertInterviewEmailTemplate. When a slot is unset (or disabled)
// the api falls back to its built-in default copy, so an empty page changes nothing.
//
// Mirrors settings/approval-emails. Imported directly from Button.tsx, not the ui-v2 barrel: the
// barrel re-exports DataTable, which pulls @tanstack/react-table's ESM build and breaks jest.
import { useEffect, useState } from 'react';
import {
  INTERVIEW_EMAIL_EVENT_TYPES,
  INTERVIEW_EMAIL_EVENT_LABELS,
  INTERVIEW_EMAIL_EVENT_VARIABLES,
  useInterviewEmailTemplates,
  useUpsertInterviewEmailTemplate,
  type InterviewEmailEventType,
  type InterviewEmailTemplateSlot,
} from '../../../../../lib/hooks/useInterviewEmailTemplates';
...
function blankSlot(eventType: InterviewEmailEventType): InterviewEmailTemplateSlot {
  return { eventType, subject: '', body: '', enabled: true };
}

function EventSection({ slot, onSaved }: { slot: InterviewEmailTemplateSlot; onSaved: (label: string) => void }) {
  const label = INTERVIEW_EMAIL_EVENT_LABELS[slot.eventType];
  const variables = INTERVIEW_EMAIL_EVENT_VARIABLES[slot.eventType];
  const upsert = useUpsertInterviewEmailTemplate();
...
export default function V2InterviewEmailsSettingsPage() {
  const { data, isLoading, isError } = useInterviewEmailTemplates();
  const [notice, setNotice] = useState<Notice>(null);
  const notify = (type: 'success' | 'error', text: string) => { setNotice({ type, text }); setTimeout(() => setNotice(null), 4000); };

  const slots: InterviewEmailTemplateSlot[] = INTERVIEW_EMAIL_EVENT_TYPES.map(
  ...
  return (
    <div style={{ maxWidth: 780 }}>
      <div style={{ marginBottom: 16 }}>
        <h1 className="v2-title" style={{ fontSize: 22, margin: 0 }}>Interview Emails</h1>
        <p style={{ ...desc, marginTop: 6 }}>Customize the emails sent to candidates, panelists, and recruiters as interviews are scheduled, confirmed, and cancelled. Leave an event disabled or blank to use the built-in default copy.</p>
      </div>
      ...
      {isError && <p style={{ fontSize: 13, color: 'var(--danger)' }}>Failed to load interview email templates.</p>}
