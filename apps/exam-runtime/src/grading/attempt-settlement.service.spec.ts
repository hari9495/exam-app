  ...
  describe('finalize', () => {
    ...
    it('auto-zeroes an essay question the candidate never answered instead of queueing it for a human', async () => {
      ...
      expect(tx.answer.create).toHaveBeenCalledWith({
        data: {
          attemptId: 'attempt-1', questionId: 'q2', selectedOptionIdsJson: '[]', answerText: null, answerFilesJson: null,
    ...
    it('routes an attempted file_upload question (files uploaded) to pending_manual_grade', async () => {
      const attempt = { id: 'attempt-1', candidateId: 'cand-1', examId: 'exam-1', questionOrderJson: JSON.stringify(['q1', 'q2']) };
      const tx = {
        question: {
          findMany: jest.fn().mockResolvedValue([
            { id: 'q1', type: 'single_mcq', marks: 5, negativeMarks: 0, options: [{ id: 'opt-a', isCorrect: true }, { id: 'opt-b', isCorrect: false }] },
            { id: 'q2', type: 'file_upload', marks: 10, negativeMarks: 0, options: [] },
          ]),
        },
        answer: {
          findMany: jest.fn().mockResolvedValue([
            { id: 'answer-1', questionId: 'q1', selectedOptionIdsJson: JSON.stringify(['opt-a']) },
            // A file_upload attempt is "attempted" via a non-empty answerFilesJson, not answerText.
            { id: 'answer-2', questionId: 'q2', selectedOptionIdsJson: '[]', answerText: null, answerFilesJson: JSON.stringify([{ id: 'f1', path: 'p', fileName: 'a.pdf', contentType: 'application/pdf', size: 10 }]), marksAwarded: null },
          ]),
          update: jest.fn(),
        },
        result: { findUnique: jest.fn().mockResolvedValue(null), create: jest.fn() },
      ...
      // Only the MCQ is auto-graded; the attempted file_upload is left null for a human.
      expect(tx.answer.update).toHaveBeenCalledTimes(1);
      expect(tx.answer.update).toHaveBeenCalledWith({ where: { id: 'answer-1' }, data: { isCorrect: true, marksAwarded: 5 } });
      expect(result.status).toBe('pending_manual_grade');
    });
    ...
    it('auto-zeroes a file_upload question with no files uploaded instead of queueing it for a human', async () => {
      ...
      const tx = {
        ...
        answer: {
          findMany: jest.fn().mockResolvedValue([
            { id: 'answer-1', questionId: 'q1', selectedOptionIdsJson: JSON.stringify(['opt-a']) },
            // A row exists (markedForReview was toggled) but no files were uploaded -> not attempted.
            { id: 'answer-2', questionId: 'q2', selectedOptionIdsJson: '[]', answerText: null, answerFilesJson: JSON.stringify([]), marksAwarded: null },
          ]),
          update: jest.fn(),
          create: jest.fn(),
        },
      ...
      // Existing empty row -> updated (not created) with the auto-zero + explanation.
