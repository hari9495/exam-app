...
describe('CandidateWhatsappTemplatesController', () => {
  let controller: CandidateWhatsappTemplatesController;
  let service: {
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      controllers: [CandidateWhatsappTemplatesController],
      providers: [{ provide: CandidateWhatsappTemplatesService, useValue: service }],
    })
      ...
      .useClass(MockGuard)
      .compile();
    controller = moduleRef.get(CandidateWhatsappTemplatesController);
  });
  ...
  it('PATCH /:id delegates to upsert with the id merged into the dto', async () => {
    const dto = { name: 'Offer v2', triggerStageId: 'st-offer', triggerMode: 'prompt', body: 'B' } as any;
  ...
  it('is unreachable when JwtAuthGuard rejects the request', async () => {
    ...
    const response = await request(server).get('/candidate-whatsapp-templates');
