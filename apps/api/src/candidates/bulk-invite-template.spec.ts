...
describe('generateBulkInviteTemplate', () => {
  it('produces a workbook with the Email/First Name/Last Name/Phone headers and one example row', async () => {
    ...
    expect(headers).toEqual(['Email', 'First Name', 'Last Name', 'Phone']);
    ...
    expect(String(exampleRow[3]).length).toBeGreaterThan(0);
  });
});
