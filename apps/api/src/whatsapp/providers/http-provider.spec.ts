...
describe('httpProvider.validateConfig', () => {
  ...
  it('accepts a public https url', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'https://api.example.com/whatsapp' })).not.toThrow();
  });
  ...
  it('rejects a missing bodyTemplate', () => {
    expect(() => httpProvider.validateConfig({ url: 'https://api.example.com/whatsapp' })).toThrow(
      BadRequestException,
    );
  });
  ...
  it('rejects a non-https url', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'http://api.example.com/whatsapp' })).toThrow(
      BadRequestException,
    );
  });

  it('rejects localhost', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'https://localhost/whatsapp' })).toThrow(
      BadRequestException,
    );
  });

  it('rejects 127.0.0.1 (IPv4 loopback)', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'https://127.0.0.1/whatsapp' })).toThrow(
      BadRequestException,
    );
  });

  it('rejects 10.0.0.5 (IPv4 private range)', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'https://10.0.0.5/whatsapp' })).toThrow(
      BadRequestException,
    );
  });

  it('rejects 172.16.0.1 (IPv4 private range)', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'https://172.16.0.1/whatsapp' })).toThrow(
      BadRequestException,
    );
  });

  it('rejects 192.168.1.1 (IPv4 private range)', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'https://192.168.1.1/whatsapp' })).toThrow(
      BadRequestException,
    );
  });

  it('rejects 169.254.1.1 (IPv4 link-local)', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'https://169.254.1.1/whatsapp' })).toThrow(
      BadRequestException,
    );
  });

  it('rejects [::1] (IPv6 loopback)', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'https://[::1]/whatsapp' })).toThrow(
      BadRequestException,
    );
  });
  ...
  it('accepts a public hostname starting with "fd" (not an IP literal)', () => {
    expect(() => httpProvider.validateConfig({ ...base, url: 'https://fd-whatsapp.example.com/x' })).not.toThrow();
  });
...
describe('httpProvider.send', () => {
  ...
  it('substitutes {{to}}/{{body}} into the body and url, defaults method to POST and content-type to json', async () => {
    ...
    const config = {
      url: 'https://api.example.com/whatsapp?to={{to}}',
    ...
    expect(url).toBe(`https://api.example.com/whatsapp?to=${args.to}`);
  ...
  it('JSON-escapes a body value containing a quote when contentType is json', async () => {
    const fetchImpl = mockFetch({ ok: true, status: 200 });
    const config = { url: 'https://api.example.com/whatsapp', bodyTemplate: '{"body":"{{body}}"}' };
  ...
  it('returns ok:true for a 2xx response', async () => {
    ...
    const result = await httpProvider.send(
      { url: 'https://api.example.com/whatsapp', bodyTemplate: '{}' },
      args,
      fetchImpl as never,
    );
  ...
  it('calls fetch with redirect:manual so a 3xx to an internal address is never followed', async () => {
    const fetchImpl = mockFetch({ ok: true, status: 200 });
    await httpProvider.send({ url: 'https://api.example.com/whatsapp', bodyTemplate: '{}' }, args, fetchImpl as never);
    const [, options] = fetchImpl.mock.calls[0];
  ...
  it('re-validates the final substituted url and never calls fetch when {{to}} lands in the host', async () => {
    const fetchImpl = mockFetch({ ok: true, status: 200 });
    const config = { url: 'https://{{to}}/x', bodyTemplate: '{}' };
