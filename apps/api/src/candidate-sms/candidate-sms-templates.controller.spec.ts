...
describe('CandidateSmsTemplatesController', () => {
  let controller: CandidateSmsTemplatesController;
  let service: {
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [{ provide: CandidateSmsTemplatesService, useValue: service }],
    })
      ...
      .useClass(MockGuard)
      .compile();
    controller = moduleRef.get(CandidateSmsTemplatesController);
  });
  ...
  it('POST / delegates to upsert with the dto', async () => {
    const dto = { name: 'Offer', triggerStageId: null, triggerMode: 'prompt', body: 'B' } as any;
  ...
  it('is unreachable when JwtAuthGuard rejects the request', async () => {
    ...
    const response = await request(server).get('/candidate-sms-templates');
