...
export class CandidateEmailsService {
  ...
  async sendMessage(
    ...
    const prepared = await this.tenantPrisma.forTenant(context, async (tx) => {
      ...
      return { skipped: false as const, entry, applicationToken, org, actorName, actorSignature, unsubscribeToken };
    });
    if (prepared.skipped) return null;
    const { entry, applicationToken, org, actorName, actorSignature, unsubscribeToken } = prepared;
