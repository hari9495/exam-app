...
**Correction (this claim was wrong when first written):** this line originally claimed "all Phase 1b items closed with full narrative comments from the start." That was false — an audit at the end of Phase 1c found all 4 of these User Stories AND Feature #5778 itself had been closed with zero comments, the exact same mistake as Phase 1a. The mistake recurred a second time in Phase 1c too (all 6 of its User Stories had zero comments). All 10 User Stories + Feature #5778 were backfilled at that point. **The lesson written after Phase 1a did not actually change behavior for two more phases — writing it down was not sufficient.** See the strengthened guidance in Section 8.
...
5. **Phase 1d (Exam-Taking Runtime) is next** per the Phase 1 decomposition (Section 6). Before starting it or any new work: this project follows a structured process — brainstorm/design first (`superpowers:brainstorming`), write an implementation plan (`superpowers:writing-plans`), then execute task-by-task with a fresh subagent per task + independent review + fix loops (`superpowers:subagent-driven-development`). **Note the actual precedent set by all phases so far: work happens directly on `main`, not an isolated worktree/feature branch** — this deviates from the skill's own stated default, but matches how this specific project has always been run; confirm with the user if picking this up in a context where that might have changed. Create the new sub-phase as a `Feature` work item under Epic #5764 (not a new Epic — see the Phase 1a duplicate-Epic lesson in Section 6), with User Stories/Tasks under that Feature.
   - **On the narrative-comment lesson specifically: writing it down here did not work.** It was written after Phase 1a, then silently violated in both Phase 1b and Phase 1c (all their User Stories, plus one Feature, closed with zero comments each time) and only caught by an explicit end-of-phase audit query, not by remembering the rule while working. **Do not rely on remembering to comment each item as you close it. Instead, run this exact verification query after closing out ANY phase, before reporting it done:**
     ```bash
     ...
     ```
     An empty result is the only acceptable outcome. If it returns rows, backfill every one of them before considering the phase closed.
